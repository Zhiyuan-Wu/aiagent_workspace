...
#### 1. 背景
当前 `alpha_mining/pairwise_rank.py` 实现了一个基础的多层感知机（MLP）模型，用于根据输入特征（alpha因子）预测股票收益排序并构建投资组合。该实现较为简单，缺乏现代深度学习中提升性能与稳定性的关键技术。为系统性地探索在A股市场上表现更优的深度学习架构，制定本实验计划。采用**贪心搜索策略**：每个新实验均基于迄今为止回测年化收益率最高的配置，并仅改变一个维度的设置，以清晰评估该修改对投资绩效的影响。
...
- **因子、回测配置、排序方法、数据生成参数**: 无特殊需求时，保持现有程序的默认值。
